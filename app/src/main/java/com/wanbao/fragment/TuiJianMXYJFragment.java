...
import com.wanbao.modle.Money_Amount;
import com.wanbao.modle.OkObject;
import com.wanbao.viewholder.TuiJianYJViewHolder;
 ...
 */
public class TuiJianMXYJFragment extends PSFragment implements SwipeRefreshLayout.OnRefreshListener {
    ...
    Unbinder unbinder;
    private View view;
    private RecyclerArrayAdapter<Money_Amount.DataBean> adapter;
    private String id;
    int page = 1;

    public static TuiJianMXYJFragment newInstance(String param2) {
        TuiJianMXYJFragment fragment = new TuiJianMXYJFragment();
    ...
    private void initRecycler() {
        ...
        recyclerView.setAdapterWithProgress(adapter = new RecyclerArrayAdapter<Money_Amount.DataBean>(context) {
            @Override
            public BaseViewHolder OnCreateViewHolder(ViewGroup parent, int viewType) {
                int layout = R.layout.item_yj;
                return new TuiJianYJViewHolder(parent, layout);
            }
        });
            ...
            public void onMoreShow() {
                HttpApi.post(context, getOkObjectOrder(), new HttpApi.CallBack() {
                    ...
                    public void onSuccess(String s) {
                        try {
                            page++;
                            Money_Amount usercar_index = GsonUtils.parseJSON(s, Money_Amount.class);
        ...
        HttpApi.post(context, getOkObjectOrder(), new HttpApi.CallBack() {
            ...
            public void onSuccess(String s) {
                try {
                    ...
                    int status = usercar_index.getStatus();
                    if (status == 1) {
                        image.setImageDrawable(ContextCompat.getDrawable(context, R.mipmap.icon_zqyj));
                        text.setText(usercar_index.getAmount() + "");
                        adapter.clear();
                        adapter.addAll(usercar_index.getData());
